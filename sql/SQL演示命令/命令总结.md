...
- 默认情况下对于任何的数据查询，mysql都会从`按顺序的第一行数据一直扫描到最后一行数据`，遇见`符合条件的数据`就将其加入查询结果集中，`表越大，花费时间就越长`。

- 想要快速地查询出数据怎么办呢？`这时候就可以使用索引。`

## (二、)索引

### 1.定义
  索引是建立在`表中列上的一个数据库对象`，是一种`具备快速定位的数据结构。`

### 2.作用
  在表中可以`给一列或者多列创建设置索引`，如果给一个列{字段}设置了索引，那么`查询的时候会先去索引列表中查询`，`而非对整个表查询`，由于索引是一种`十分高效的数据结构`，就会大大提升查询速度。

### 3.原理
  索引是`B+树`的底层数据结构，所以时间复杂度是`log2n`，可以直接定位到特定行。
 
### 4.举例
  例如一个表中有 10 万行数据，现在要执行这样一个查询语句:select*from table where id=10000;。如果不使用索引，那么就必须从头开始遍历整个表，`直到找到id 等于 10 000 的这一行数据`。但如果为id 字段设置了一个索引，则不需要遍历整个表，使用`近似二分法的效率`，可以直接在索引里找到 10000, 然后得到这一行的数据。你没有看错，索引可以直接定位到第 10 000 行，而不需要从第一行数据开始逐行扫描。
...
实例：
下面是1百万条数据，现在要查询id为1000000的记录，使用索引和不使用索引的时间对比。

https://www.doubao.com/thread/wd89fa27bd5b448ea
...
-- 如果a不为null，则返回a结果，否则返回b

```

题目：返回第二高的薪资，如果不存在返回null，要求去重。
https://leetcode.cn/problems/second-highest-salary
```sql
select ifnull(
    (
     select distinct salary  -- 这里又有了一个去重的distinct命令
     from  Employee 
     order by salary 
...
### 2.函数
https://chat.deepseek.com/share/n0un4mtiz4yrk3kj0h

在正式讲解函数之前，我们先看看mysql中如何使用函数的，这样能够先`直观感受到函数`。
...
```sql
    ...
    BEGIN
        RETURN 'Hello, World!';
    END$$ -- 自定义分隔符，这里用$$代替默认的分号,到这里，这个函数才结束！
```

##### 终端中函数写法，多了分隔符
然而在终端中，我们常常不能像在客户端一样方便，需要自定义分隔符，防止mysql运行到( 'Hello, World!';)后面的分号时就认为语句结束了。

因此，一般，在终端中，是如下写法

```sql
    -- 创建函数前设置分隔符（避免分号冲突）【即再遇到DELIMITER之前，所有的分号都不会被解释为结束语句，而是作为普通的字符处理】
    DELIMITER $$ -- 自定义分隔符，这里用$$代替默认的分号
...
```sql
    ...
    SELECT hello_world();  -- 输出结果，Hello, World!
...
```sql
    ...
    routine_body
```
具体实例，请看此文：https://chat.deepseek.com/share/4yw4y5xm99n44dsw7z
我们要注意的是：

##### 2.3 必须拥有的部分
- CREATE FUNCTION function_name：创建函数，并指定函数名
- RETURNS return_type：指定函数的返回类型
- routine_body：函数的主体，包含具体的逻辑实现

例如：
```sql
CREATE FUNCTION add_numbers(a INT, b INT) 
RETURNS INT
DETERMINISTIC --含义是确定的，相同输入，必定相同输出
-- ↓↓↓ routine_body 开始 ↓↓↓
BEGIN
    -- 声明变量（就像准备容器）
    DECLARE result INT;
...
- [DEFINER = user]：指定函数的定义者
- [characteristic ...]：函数的特性，如 DETERMINISTIC（确定性）或 NOT DETERMINISTIC（非确定性）

例如：
```sql
    DELIMITER $$ 
    -- 创建一个"工资计算器"
    CREATE OR REPLACE                             -- 如果有就替换(8.1版本才有的，5.7版本不用管)
    -- DEFINER = 'hr_admin'@'localhost'              -- 属于HR管理员
    FUNCTION add_numbers(a INT, b INT) 
    RETURNS INT
    DETERMINISTIC                                 -- 特性：相同输入相同输出
    READS SQL DATA                                -- 特性：会读数据库（比如奖金比例表）
    SQL SECURITY DEFINER                          -- 特性：用定义者权限
    COMMENT '加法'           -- 特性：功能说明
    -- ↓↓↓ routine_body 开始 ↓↓↓
    BEGIN
        -- 声明变量（就像准备容器）
